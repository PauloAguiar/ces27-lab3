    ...
    for {
        ...
        case rv := <-raft.requestVoteChan:
            ...
            if rv.Term < raft.currentTerm && (rv.Term == raft.currentTerm || raft.votedFor == 0) {
                ...
                log.Printf("[FOLLOWER] Voted '%v' for term '%v'.\n", raft.peers[rv.CandidateID], raft.currentTerm)
        ...
        case ae := <-raft.appendEntryChan:
            ...
            //Will only reject the appendEntry if the term is smaller
            if ae.Term < raft.currentTerm {
                log.Printf("[FOLLOWER] Rejected AppendEntry from '%v'. Term: '%v'\n", raft.peers[ae.LeaderID], raft.currentTerm)
                reply.Success = false
                ae.replyChan <- reply
                break
    ...
    for {
        ...
        case rvr := <-replyChan:
            ...
            if rvr.VoteGranted {
                ...
                //Check if the majority voted for this
                if voteCount > len(raft.peers)/2 {
                    log.Printf("[CANDIDATE]  %v will be Leader", raft.peers[raft.me])
                    raft.resetElectionTimeout()
        ...
        case ae := <-raft.appendEntryChan:
            ...
            if ae.Term < raft.currentTerm {
                //Doesn't accept even though he is a candidate
